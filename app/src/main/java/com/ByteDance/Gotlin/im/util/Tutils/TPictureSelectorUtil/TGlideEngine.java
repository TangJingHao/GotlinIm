...
public class TGlideEngine implements ImageEngine {
    /**
     * 加载图片
     *
     * @param context   上下文
    ...
    public void loadAlbumCover(@NonNull Context context, @NonNull String url, @NonNull ImageView imageView) {
        ...
        Glide.with(context)
                ...
                .placeholder(R.drawable.t_ps_image_placeholder)
                .into(imageView);
    }
    ...
    private TGlideEngine() {
    }

    private static TGlideEngine instance;

    public static TGlideEngine createGlideEngine() {
        if (null == instance) {
            synchronized (TGlideEngine.class) {
                if (null == instance) {
                    instance = new TGlideEngine();
                }
            }
        }
        return instance;
